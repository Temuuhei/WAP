...
async function getProducts() {
    let products = await fetch('http://localhost:3100/products/').then(response => response.json());
...
function renderProduct(prod) {
    ...
    deleteBtn.addEventListener('click', function(event) {
        ...
        fetch('http://localhost:3100/products/' + prod.id, {
            method: 'DELETE',
        }).then(response => {
...
function editProduct() {
    ...
    fetch('http://localhost:3100/products/' + prodId, {
            method: 'PUT',
            headers: {
                'Content-type': 'application/json',
            },
